...
namespace MarcosPereira.Terrain {
    ...
    public class ClampNode : BaseNode {
        ...
        public override Task<List<float>> Execute(
            ...
            for (int i = 0; i < inputs[0].Count; i++) {
                if (inputs[0][i] < this.min) {
                    results.Add(this.min);
                } else if (inputs[0][i] > this.max) {
    ...
    public class ClampNodeUI : BaseNodeUI {
        protected override string Description =>
            "Clamp input values to a given range.";
    ...
    public static class ClampNodeExtensions {
        public static IModelUI CreateNode(
            this ElementBuilder elementBuilder,
            CommandDispatcher dispatcher,
            ClampNode model
        ) {
            IModelUI ui = new ClampNodeUI();
